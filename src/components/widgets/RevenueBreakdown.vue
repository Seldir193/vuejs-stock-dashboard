...
const ttm = computed(() => {
  ...
  companies.forEach(({ name, ticker }) => {
    ...
    r[name] = last4.reduce((a, b) => a + (b?.value ?? 0) / 1_000, 0)
  })
  return r
})
...
const latestQuarter = computed(() => {
  ...
  companies.forEach(({ name, ticker }) => {
    ...
    m[name] = last?.period ? toQuarterLabel(last.period) : '—'
  })
  return m
})
...
const TOOLTIP = {
  ...
  intersect: true,
  displayColors: false,
  callbacks: { title: ttTitle, label: ttLabel, footer: ttFooter }
}
